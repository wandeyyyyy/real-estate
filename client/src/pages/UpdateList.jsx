...
const handleSubmitForm = async (e) => {
    ...
    try {
        ...
        if(formData.images.length > 6) return setError("Upload atmost 6 image")
...
<input type="text" id="name" placeholder='Name' className='border p-3 rounded-lg  outline-none' maxLength="62" minLength="10"  onChange={handleChanges} value={formData.name} />
<textarea type="text"  id="description" placeholder='Description' className='border p-3 rounded-lg  outline-none'  onChange={handleChanges} value={formData.description}/>
<input type="text"  id="address" placeholder='Address' className='border p-3 rounded-lg  outline-none'  onChange={handleChanges} value={formData.address}/>
...
<div className='flex flex-wrap gap-6 font-semibold'>
    <div className='flex items-center gap-2'>
        <input type="number" name="" id="bedrooms" min="1" max="10" className='border border-gray-300 rounded-lg p-3 outline-none' onChange={handleChanges} value={formData.bedrooms}  />
        <span>Beds</span>
    </div>
    <div className='flex items-center gap-2'>
        <input type="number" name="" id="bathrooms" min="1" max="10" className='border border-gray-300 rounded-lg p-3 outline-none ' onChange={handleChanges} value={formData.bathrooms}  />
        <span>Baths</span>
    </div>
  
    <div className='flex items-center gap-2'>
        <input type="number"  id="regularPrice" min="50" max="1000000" className='border border-gray-300 rounded-lg p-3 outline-none'onChange={handleChanges} value={formData.regularPrice} />
...
<span className='text-xs'>($/ Month)</span>
    ...
    {formData.offer &&
    <div className='flex items-center gap-2'>
        <input type="number"  id="discountPrice" min="0" max="1000000" className='border border-gray-300 rounded-lg p-3 outline-none'onChange={handleChanges} value={formData.discountPrice} />
        <div className='flex flex-col items-center'>
...
<button type='submit' disabled={ loads || loading } className='p-3 bg-slate-700 rounded-lg hover:opacity-95 disabled:opacity-80 text-white font-semibold'>{loads ? "UPDATING..." : "UPDATE LISTING"}</button>
